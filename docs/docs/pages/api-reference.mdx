...
### `generatePresharedKey(options?: object)`

Generates a WireGuard preshared key for hardening peer tunnels.

**Parameters:**
- `options` (`object`, optional):
  - `keyPath` (`string`, optional): Path to write the generated preshared key (default: `/etc/wireguard/presharedkey`)

**Returns:** `Promise<{presharedkey: string}>`

**Example:**
```ts
import { generatePresharedKey, addPeer } from "@kriper0nind/wg-utils"
...
- `options` (`object`): Configuration options
  ...
  - `dns` (`string`, optional): DNS server pushed to clients
  - `postUp` (`string[]`, optional): Custom commands executed after the interface starts (replaces defaults)
  - `postDown` (`string[]`, optional): Custom commands executed when the interface stops
  - `mtu` (`string`, optional): Interface MTU value
  - `table` (`"auto" | "off" | string`, optional): Routing table behavior
...
- `options` (`object`): Peer configuration
  ...
  - `persistentKeepalive` (`number`, optional): NAT keepalive interval (seconds)
  - `presharedKey` (`string`, optional): Preshared key from `generatePresharedKey`
  - `allowedIPs` (`string`, optional): Custom AllowedIPs string (defaults to next `/32`)
  - `endpoint` (`string`, optional): Peer endpoint (`host:port`)
  - `allowedApps` (`string`, optional): Pass-through metadata stored as `AllowedApps`
...
interface GeneratePresharedKeyResult {
  presharedkey: string
}
